...
int main (int argc, char* argv[])
{
    ...
    if (taskid == MASTER) { // Master receives position of points.
        ...
        while (times < total_times) { 
            ...
            int start;
            // #pragma omp parallel for num_threads(n_thread) private(recv_average_tasks, start)
        ...
        while (Stimes < total_times) {
            ...
            int has_collided;
            double Fx, Fy, mass;
            // #pragma omp parallel for num_threads(n_thread) private(Fx, Fy, mass, has_collided)
            for(int i = start; i < average_tasks+start; i++) {
                ...
                // #pragma omp parallel num_threads(n_thread) reduction(+:Fx) reduction(+:Fy) private(r, fx, fy, mass2, has_collided)
...
void printInfo(double runningTime) {
    printf("\nName: %s\n", "Liang Jialu");
    printf("Student ID: %d\n", 118010164);
    printf("Assignment 3, N Body, MPI+openMP implementation.\n");
    printf("runTime is %f\n", runningTime);
